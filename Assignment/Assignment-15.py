...
Apply Butterworth, Gaussian, and Ideal filtering (low-pass, high-pass, band-pass) on grayscale images with low, normal, and high contrast.
...
def main():
    ...
    # Load contrast variants 
    low_img = cv2.imread(r"E:\BSC_In_CSE\4th_Year_1st _ememster\Digital_Image_Processing\Assignment\outputs\dog_3_low_contrast.jpg", 0)
    norm_img = cv2.imread(r"E:\BSC_In_CSE\4th_Year_1st _ememster\Digital_Image_Processing\Assignment\outputs\dog_3_normal.jpg", 0)
    high_img = cv2.imread(r"E:\BSC_In_CSE\4th_Year_1st _ememster\Digital_Image_Processing\Assignment\outputs\dog_3_high_contrast.jpg", 0)
...
def butterworth_filter(shape, type='low', n=2):
    radius_low = shape[0] // 4
    radius_high = shape[0] // 6
    ...
    if type == 'low':
        ...
        return butterworth_low_pass_filter(shape, cutoff=radius_high, n=n) - butterworth_low_pass_filter(shape, cutoff=radius_low, n=n)
...
def gaussian_filter(shape, type='low'):
    ...
    if type == 'low':
        ...
        return gaussian_low_pass_filter(shape, cutoff=radius_high) - gaussian_low_pass_filter(shape, cutoff=radius_low)
...
def ideal_filter(shape, type='low'):
    ...
    if type == 'low':
        ...
        return ideal_low_pass_filter(shape, cutoff=radius_high) - ideal_low_pass_filter(shape, cutoff=radius_low)
